                ...
                <div class="col s12 border-bottom-1 mt-5">
                  <ul class="tabs">
                    ...
                    <li class="tab col s6">
                  	...
                  	<li class="indicator" style="right: 185px; left: 0px;"></li>
                  </ul>
                </div>
                  ...
                  <ul class="collection border-none">
                    ...
                    <li class="collection-item border-none">
                      <div class="m-0">
                        ...
                        <div class="switch right">
                          <label>
                            <input class="recentActivityBtn" <%= DAL_Activity.GetStatus() %> type="checkbox">
                            <span class="lever"></span>
                          </label>
                        </div>
                      </div>
		...
		<script>
			$(document).ready(function() {
				$("input.recentActivityBtn").change(function() {
					if($(this).is(":checked")) {
						$.ajax({
							type: "POST",
							url: "/webservices/settingsService.asmx/UpdateSetting",
							data: "{name:'recent-activity', value:'true'}", 
						...
						$.ajax({
							type: "POST",
							url: "/webservices/settingsService.asmx/UpdateSetting",
							data: "{name:'recent-activity', value:'false'}", 
							contentType: "application/json; charset=utf-8", // we are sending in JSON format so we need to specify this
							dataType: "json", // the data type we want back.  The data will come back in JSON format
							success: function (data) {
								//$("#searchresultsB").html(data.d); // it's a quirk, but the JSON data comes back in a property called "d"; {"d":"Hello Aidy F"}
