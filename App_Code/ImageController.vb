...
Public Class ImageBrowserController
    ...
    Private Const DefaultFilter As String = "*.png,*.gif,*.jpg,*.jpeg"
    Private Const ThumbnailHeight As Integer = 80
    Private Const ThumbnailWidth As Integer = 80
    Private ReadOnly directoryBrowser As DirectoryBrowser
    Private ReadOnly contentInitializer As ContentInitializer
    Private ReadOnly thumbnailCreator As ThumbnailCreator
    ...
    Public Overridable Function AuthorizeThumbnail(ByVal path As String) As Boolean
        Return CanAccess(path)
    End Function

    <OutputCache(Duration:=3600, VaryByParam:="path")>
    Public Overridable Function Thumbnail(ByVal path As String) As ActionResult
        path = NormalizePath(path)

        If AuthorizeThumbnail(path) Then
    ...
    Private Function CreateThumbnail(ByVal physicalPath As String) As FileContentResult
        Using fileStream = System.IO.File.OpenRead(physicalPath)
            Dim desiredSize = New ImageSize With {
                .Width = ThumbnailWidth,
                .Height = ThumbnailHeight
            }
            Const contentType As String = "image/png"
    ...
    Public Function Image(ByVal path As String) As ActionResult
        path = NormalizePath(path)

        If AuthorizeImage(path) Then
            Dim physicalPath = Server.MapPath(path)
    ...
    Public Overridable Function AuthorizeImage(ByVal path As String) As Boolean
