...
'Dim s = http_context.Request.GetBufferlessInputStream(True)
	...
	For loop1 = 0 To arr1.Length() - 1
		...
		memFile.SaveAs(Server.MapPath("~/files/" & path) & "\" &
...
Public Sub Read(path As String)
	...
	Dim files As String() = System.IO.Directory.GetFiles(Server.MapPath("~/files/" & path), "*.*")
	...
	Dim directory As String() = System.IO.Directory.GetDirectories(Server.MapPath("~/files/"  & path), "*.*")
...
Public Sub Thumbnail(path As String)
	...
	Dim ms As New MemoryStream(System.IO.File.ReadAllBytes(System.Web.Hosting.HostingEnvironment.MapPath("~/files/") & fileName))
	...
	'HttpContext.Current.Response.BinaryWrite(System.IO.File.ReadAllBytes(System.Web.Hosting.HostingEnvironment.MapPath("~/files/") & fileName))
...
Public Sub Create(path As String, name As String, type As String, size As String)
    ...
    Dim physicalPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/files/" & path) & name
