...
Public Class MapService
    ...
    Public Sub GetPageList()
        ...
        Try
            Dim sConnString As String = ConfigurationManager.ConnectionStrings("WebApp").ConnectionString
            Dim odbcConn As OdbcConnection = New OdbcConnection(sConnString)
            Dim sQueryString As String = "SELECT * FROM tblftb ORDER by pageName asc"
            Dim DBCommand As New OdbcCommand(sQueryString, odbcConn)
            odbcConn.Open()
...
Public Function GetNewsGUID(ByVal nNum As String) As String
	...
	Try
		Dim sConnString As String = ConfigurationManager.ConnectionStrings("WebApp").ConnectionString
		Dim odbcConn As OdbcConnection = New OdbcConnection(sConnString)
		Dim sQueryString As String = "SELECT guid FROM tblnewsftb where nNum = " & nNum
		Dim DBCommand As New OdbcCommand(sQueryString, odbcConn)
		odbcConn.Open()
		...
		Try
			...
			While odbcReader.Read()
				...
				Rslt = Trim(odbcReader("guid").ToString)
			End While
...
End Function
    ...
    <WebMethod> _
        Public Sub AddActivity(title As String, user As String, userID As String, guid As String, action As String)


		If Not guid.Contains("-") = True Then
       		guid = GetNewsGUID(guid)
        End If
		...
		Dim xmlString As String = "{ Title: '" & title & "', user: '" & user & "', action: '" & action & "',  guid: '" & guid & "', ItemDate: '" & myDate.ToString("yyyy/MM/dd") & "'}"
    ...
    <WebMethod> _
    Public Sub DeletePage(models As String)
        ...
        Try
                ...
                sQueryString = "DELETE FROM tblftb WHERE ftbNum = " & ftbNum
